...
Dataset-agnostic core; per-dataset CLI wrappers live under
``tests/<dataset>/qc_registration.py`` and just supply default paths.

Operates on a directory of template-space per-subject NIfTIs named
...
at the same voxel locations); subjects with low mean similarity to
the cohort are candidates for a mis-registration that bypassed the
bit-equality / pipeline-output sanity checks.

Per modality, :func:`run`:
...
If an external reference template is supplied via ``template_path``,
also computes MI-to-template per subject. The cohort-z vs template-z
scatter then disambiguates "registration failure" (both low) from
"atypical biology" (cohort low, template normal). This only carries
information when the template is *independent* of the cohort — i.e.
not the cohort-averaged template that registration was driven toward.
"""
...
def run(dest, mask_path=None, template_path=None,
    ...
    """Run the cohort QC and write outputs under ``output_dir``.

    Args:
        dest: directory of ``<sbj>_<mod>.nii.gz`` files.
        mask_path: brain mask NIfTI; defaults to ``<dest>/group_mask.nii.gz``.
        template_path: optional external reference template NIfTI for
            the MI-to-template metric. Pass ``None`` (or a non-existent
            path) to skip. The metric only carries information when the
            template is independent of the cohort.
        output_dir: where to write outputs; defaults to ``<dest>/qc/``.
        z_threshold: ``|z| >`` this on cohort similarity flags a subject.

    Returns:
        DataFrame with one row per (subject, modality).
    """
    dest = Path(dest)
    ...
    if template_path is not None:
        ...
        print("no template provided; skipping MI-to-template metric")

    by_mod = discover(dest)
    if not by_mod:
        raise SystemExit(
            f"No <sbj>_<mod>.nii.gz files matched in {dest}. "
            f"Filenames must look like 'sub001_fa.nii.gz'."
        )
    ...
    for modality, subject_paths in by_mod.items():
        ...
        # Per-subject MI to the reference template — disambiguates "low
        # cohort similarity because of registration failure" (low MI
        # too) from "low cohort similarity because of atypical biology"
        # (MI to template is fine; subject is anatomically placed
        # correctly but with an unusual signal pattern).
    ...
    if flagged.empty:
        ...
        for _, row in flagged.sort_values(["modality", "z_score"]).iterrows():
            ...
            # template ~normal -> likely atypical biology (image is
            # placed correctly but signal pattern differs from peers).
            if np.isnan(zt):
